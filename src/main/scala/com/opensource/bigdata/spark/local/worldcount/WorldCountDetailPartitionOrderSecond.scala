...
object WorldCountDetailPartitionOrderSecond {
  ...
  def main(args: Array[String]): Unit = {
   ...
   val r4 = r3.reduceByKey(_+_,2)
    ...
    println(s"结果:${r5Sort.collect().mkString}")
    println(s"结果:${r5Sort.map(x => (x._2,x._1) ).sortByKey().map(x => (x._2,x._1) ).collect().mkString }")
