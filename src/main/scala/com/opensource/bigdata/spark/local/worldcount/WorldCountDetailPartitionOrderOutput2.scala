...
object WorldCountDetailPartitionOrderOutput2 {
  ...
  /**
    ...
    */
  var master = "local[1]" //本地模式:local  local[2]    standalone:spark://master:7077
  ...
  def main(args: Array[String]): Unit = {
    ...
    /**
      ...
      */
    val r1:org.apache.spark.rdd.RDD[String] = sc.textFile("/home/data/input/a.text")
    println("===================")
    println(r1)
    val r2 = r1.flatMap(_.split(" "))
    val r3 =  r2.map((_,3))
   val r4 = r3.reduceByKey({
     (a,b) =>  {
       val c = a + b
       val threadName = Thread.currentThread().getName
       println(s"println_${threadName} reduceByKey  a:${a} + b:${b} = ${c}")
       c
     }

   },2)
    val r5Sort = r4.sortByKey()
    r5Sort.saveAsTextFile("file:///d:/home/data/output/3")
    println(s"结果:${r5Sort}")
