...
object Run2 extends BaseSparkSession{
  ...
  object CustomerSum extends UserDefinedAggregateFunction{
    ...
    def bufferSchema: StructType = {
      StructType(StructField("salary",LongType) ::StructField("count",LongType)  :: Nil)
    }
    ...
    //计算最终的结果
    def evaluate(buffer: Row): Long = buffer.getLong(0) / buffer.getLong(1)
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.udf.register("customerMax",CustomerSum)

    val df = spark.read.json("hdfs://standalone.com:9000/home/liuwen/data/employees.json")
    df.createOrReplaceTempView("employees")
    val sqlDF = spark.sql("select customerMax(salary)  as averageSalary from employees  ")
